export {
    CookieList, DASHBOARD_APP_EDIT_PERMISSION_ID, DASHBOARD_APP_INSTALL_PERMISSION_ID, DASHBOARD_APP_READ_PERMISSION_ID, DASHBOARD_DRAG_DROP_PERMISSION_ID as DAShBOARD_DRAG_DROP_PERMISSION_ID, DASHBOARD_OPEN_APP_PERMISSION_ID,
    DASHBOARD_SET_PERMISSION_ID,
    DASHBOARD_SPEECH_RECOGNITION_PERMISSION_ID,
    DASHBOARD_WEBVIEW_PERMISSION_ID, type Cookie,
    type GetCookiesRequest,
    type HostRequest,
    type PortProcess,
    type PromptRequest,
    type PromptRequestAppInstall,
    type PromptRequestAppPermissions,
    type PromptRequestAppPlugins,
    type PromptRequestAppUpdate,
    type PromptRequestHttpPort,
    type PromptRequestIndexInstall,
    type PromptResult,
    type SpeechRecognitionStart,
    type TranscriptSegment,
    type TranscriptStatus,
    type UserResponse,
    type WebviewEvent, type WebviewHandle, type WebviewPacket,
    type WebviewRequest,
} from './extension.js';
export type { DashboardHandler } from './handler';
export {
    DragDropReadResponse,
    type DragDropFile,
    type DragDropReadRequest,
    type DragDropRequestDashboard,
    type FileData,
} from './packets.js';

