...
const urls = Object.fromEntries(files.map(async file => {
...
execaSync('bash', ['-c', `cat ./release-assets/latest-beta.json | pnpm wrangler r2 object put ${BUCKET}/app/latest-beta.json --pipe`], option);
const platforms = [
    {
        type: 'windows-x86_64',
...
const releaseData = {
    ...
    platforms: Object.fromEntries(platforms.map(async platform => {
...
await fs.writeFile('./release-assets/version-beta.json', JSON.stringify(releaseData, null, 4));
execaSync('bash', ['-c', `cat ./release-assets/version-beta.json | pnpm wrangler r2 object put ${BUCKET}/app/version-beta.json --pipe`], option);
